...
import com.dmitriid.tetrad.adapters.TetradSlackNew;
...
public class SlackServiceNew implements IManagedService {

    private final Map<String, TetradSlackNew> slacks = new HashMap<>();
    ...
    public void init(ServiceConfiguration configuration) throws ServiceException {
        ...
        for(JsonNode node : configuration.getConfiguration().at("/slack")){
            TetradSlackNew slack = new TetradSlackNew(node, transformers);
